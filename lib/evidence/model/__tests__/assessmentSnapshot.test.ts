...
describe("resolveGateDecision", () => {
  /*
   * CONTRACT REVISION 1 turned `gateDecided: boolean` into
   * `gateDecision: "coverage" | "fatal_loss" | null`. These assertions moved
   * from `toBe(true)` / `toBe(false)` to naming WHICH gate fired, because the
   * boolean could not express the one rule that matters downstream: coverage
   * BEATS fatal-loss, they emit different reason codes, and the fatal-loss
   * reason may never reach bank-facing text while a coverage status may be
   * shown to the merchant.
   */
  it("names coverage when coverage says covered", () => {
    expect(
      resolveGateDecision(
        gatesWith({
          coverage: { state: "covered_shopify", shopifyProtectStatus: "PROTECTED" },
  ...
  it("names fatal_loss when fatal loss triggered", () => {
    expect(
      resolveGateDecision(gatesWith({ fatalLoss: { triggered: true, reason: "refund_issued", messageToken: null } })),
    ).toBe("fatal_loss");
  });

  it("prefers coverage when BOTH fire — coverage beats fatal-loss", () => {
    // The rule the boolean could not carry. A covered case is shown to the
    // merchant as covered; a fatal-loss reason is merchant-UI-only and must
    // never be the label on a case Shopify is already paying for.
    expect(
      resolveGateDecision(
        gatesWith({
          coverage: { state: "covered_shopify", shopifyProtectStatus: "PROTECTED" },
          fatalLoss: { triggered: true, reason: "refund_issued", messageToken: null },
        }),
      ),
    ).toBe("coverage");
  });

  it("is NULL when a gate was never looked at", () => {
    // "Nobody looked" is not "no gate". Collapsing them is the conflation
    // `CaseGateAssessment` was built to prevent, and `gateDecision` must not
    // reintroduce it one layer up.
    expect(resolveGateDecision(NO_GATES)).toBeNull();
    expect(
      resolveGateDecision(
        buildCaseGateAssessment({
