...
describe("review items", () => {
    ...
    expect(items).toEqual([
      {
        ...
        // Contract revision 1: an `I18nToken`, not a bare key string.
        reasonToken: { key: "fixtures.reviewRequired.tds" },
  ...
  it("substitutes a generic token rather than emitting an empty reason", () => {
    ...
    expect(projectReviewItems(plan)[0].reasonToken).toEqual({
      key: REVIEW_REASON_FALLBACK_TOKEN,
    });
  });
  ...
  it("emits no English — every reason is a token key", () => {
    for (const fixture of [FIXTURE_REVIEW_REQUIRED_SAFE, FIXTURE_REVIEW_REQUIRED_NO_SAFE]) {
      for (const item of projectReviewItems(fixture.plan)) {
        expect(item.reasonToken.key).toMatch(/^[a-zA-Z0-9_.]+$/);
        expect(item.reasonToken.key).not.toMatch(/\s/);
      }
    }
  });
