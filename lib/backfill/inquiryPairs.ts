...
async function backfillOneShop(shopId: string): Promise<BackfillShopResult> {
  ...
  //    `replacePackBasedAutomationRules` to wire phase-paired per-pack rules.
  //    Per-pack rules no longer exist — setup owns exactly two rows — and
  //    that helper's `LIKE '__dd_setup__%'` delete would destroy the
  //    canonical fallback + safeguard, silently disabling a merchant's
  //    auto-pilot. Installing the sibling pack is sufficient on its own:
  //    template resolution now goes through the phase-aware
  //    `reason_template_mappings`, which routes inquiry disputes correctly
  //    without any per-pack rule.
  return {
    shopId,
