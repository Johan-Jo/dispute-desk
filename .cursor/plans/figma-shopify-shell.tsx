...
import { Shield, ThumbsUp, Star, X, Home, ShoppingCart, Package, Users, BarChart, Settings, FileText, Zap, AlertCircle, Search, Bell, LayoutDashboard, Info, CreditCard, Database, MessageSquare, HelpCircle, Activity } from 'lucide-react';
import ShopifyDisputes from './shopify-disputes';
import ShopifyDisputeDetail from './shopify-dispute-detail';
import ShopifyCases from './shopify-cases';
import ShopifyCaseDetail from './shopify-case-detail';
import ShopifyInquiryDetail from './shopify-inquiry-detail';
import ShopifyPacks from './shopify-packs';
import ShopifyRules from './shopify-rules';
import ShopifyRulesRedesign from './shopify-rules-redesign';
import ShopifyEvidenceSources from './shopify-evidence-sources';
import ShopifyInquiries from './shopify-inquiries';
...
export default function ShopifyShell({ currentPath, onNavigate, children }: ShopifyShellProps) {
  ...
  const appNavItems = [
    ...
    { label: 'Coverage', path: '/shopify/coverage', icon: Shield },
    { label: 'Automation', path: '/shopify/automation', icon: Zap },
    { label: 'Playbooks', path: '/shopify/playbooks', icon: FileText },
    { label: 'Billing', path: '/shopify/billing', icon: CreditCard },
    { label: 'Settings', path: '/shopify/settings', icon: Settings },
    { label: 'Help', path: '/shopify/help', icon: HelpCircle },
  ];
  ...
  const renderPage = () => {
    ...
    // Map new routes to existing pages
    if (currentPath === '/shopify/disputes') return <ShopifyCases onNavigate={onNavigate} />;
    if (currentPath === '/shopify/coverage') return <ShopifyPacks onNavigate={onNavigate} />;
    if (currentPath === '/shopify/automation') return <ShopifyRulesRedesign onNavigate={onNavigate} />;
    if (currentPath === '/shopify/playbooks') return <ShopifyEvidenceSources onNavigate={onNavigate} />;
    if (currentPath === '/shopify/billing') return <ShopifyPlanManagement onNavigate={onNavigate} />;
    if (currentPath === '/shopify/settings') return <ShopifySettings />;
    if (currentPath === '/shopify/help') return <div className="text-center py-12 text-[#6D7175]">Help Center (Coming soon)</div>;
    
    // Route to specific detail pages based on case ID
    if (currentPath.startsWith('/shopify/disputes/')) {
      // In a real app, you'd fetch the case type from an API
      // For demo purposes, we determine by ID pattern
      const caseId = currentPath.split('/').pop();
      // IDs ending in 4, 6, 8 are inquiries, others are disputes
      const isInquiry = caseId && (caseId.endsWith('4') || caseId.endsWith('6') || caseId.endsWith('8'));
      return isInquiry 
        ? <ShopifyInquiryDetail onNavigate={onNavigate} />
        : <ShopifyDisputeDetail onNavigate={onNavigate} />;
    }
