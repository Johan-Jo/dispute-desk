...
const KEYS = {
  ...
  // ── Risk-to-dispute correlation (Initial Analysis page) ─────────
  // Observational signal: "of orders Shopify classified X, Y%
  // became disputes." Says nothing about causation — Shopify's
  // analysis is the input, not the verdict.
  correlationTitle: "Risk classification vs. dispute outcomes",
  correlationSubtitle:
    "How often each Shopify risk level on your store has converted to an actual dispute. Observed, not predicted.",
  correlationInsufficient: "Insufficient sample",
  correlationObservationStrong:
    "On your store, orders Shopify classified as high-risk became disputes about {ratio}x more often than orders Shopify cleared or rated low-risk. This is a strong observed correlation — high-risk orders are worth manual review.",
  correlationObservationMild:
    "On your store, orders Shopify classified as high-risk became disputes about {ratio}x more often than orders Shopify cleared or rated low-risk. The signal is mild, not overwhelming.",
  correlationObservationFlat:
    "On your store, high-risk and low-risk orders have converted to disputes at similar rates so far. Shopify's classification has not provided strong predictive separation for your dataset yet.",
  correlationObservationInverted:
    "Unusual — on your store, high-risk orders have converted to disputes at a similar or lower rate than orders Shopify cleared. This can happen when the dispute sample is small or when disputes are driven by post-purchase issues (delivery, product) rather than fraud signals.",
  ...
  // Risk Intelligence KPI segments — 30d strip split into 3 cards
  segmentFraudProfileTitle: "Fraud-risk profile · Last 30 days",
  segmentFraudProfileSubtitle: "Shopify's classification signals and the disputes that followed",
  segmentVerificationTitle: "Payment verification · Last 30 days",
  segmentVerificationSubtitle: "Card-level authentication and Shopify Protect underwriting",
  segmentDeliveryTitle: "Delivery operations · Last 30 days",
  segmentDeliverySubtitle: "Carrier confirmation, signature capture, and fulfillment timing",
  ...
  kpiConfirmedDelivery: "Confirmed deliveries",
  kpiConfirmedDeliveryContext: "{num} of {den} fulfilled orders",
  kpiSignedFor: "Signed for at delivery",
  kpiSignedForContext: "{num} of {den} delivered",
