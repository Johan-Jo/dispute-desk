...
- [x] If Shopify fails the renewal charge, the merchant is emailed within 5 minutes of the webhook. (Email shipped 2026-05-15 — `sendGraceEnteredEmail` fires from `reconcileBillingCycleForShop` on the active → grace transition, idempotent via `grace_entered_at` + audit lookup. Yellow banner copy still pending — lands with Phase 4b.)
