...
| 10 | Dashboard metrics | `lib/disputes/__tests__/metrics.test.ts` (18 tests: active counts + ACTIVE_NORMALIZED filter, winRate, recoveryRate, totalClosed, avgTimeToSubmit, status/outcome breakdowns, inquiry vs chargeback, needsAttention, currency picker, period-over-period pctChange + winRateChange in pp, admin-only field nulling, chargebackRate threading) | **Manual:** open `/app` overview after seeding disputes, confirm KPI cards match `disputes` rows |
