...
/* Skip packs that already carry a canonical assessment.
 *
 * Without this a second batch re-picks the FIRST cases again (the order is by
 * dispute id, deliberately, so batches are reproducible) — rebuilding cases
 * that are already done and, worse, superseding any `failed` package they
 * carry. A human-gated rejection must not be overwritten by an automatic
 * rebuild; the guard for that is not built yet, so the batch avoids them. */
const SKIP_CANONICAL = args.includes("--skip-canonical");
const OUT =
...
if (SKIP_CANONICAL) population = population.filter((p) => !p.pack_json?.case_assessment);

/* ── HUMAN-GATED REJECTIONS ARE EXCLUDED FROM THE BATCH ────────────────
 *
 * This script enqueues `build_pack` directly, so `maybeEnqueueDefencePackage`'s
 * guard only stops the SECOND half of the chain — the pack would still be
 * rebuilt first. Excluding the case here means the whole chain declines.
 *
 * It is the same predicate the enqueue path uses, mirrored rather than
 * imported because this is a `.mjs` ops script and booting the TS toolchain
 * inside a batch selector would be worse; `latestPackageGuardParity.test.ts`
 * fails the build if the two ever disagree.
 *
 * On 2026-08-11 this batch rebuilt `1cc88617` — a case whose newest package
 * was a validation rejection — because the protection was a manual
 * instruction and the selection basis changed underneath it. */
const blockedCaseIds = new Set();
{
  const rows = await page(
    "defence_packages?select=dispute_id,version,status,validation_status&order=version.desc",
  );
  const latestByCase = new Map();
  for (const r of rows) {
    if (r.dispute_id && !latestByCase.has(r.dispute_id)) latestByCase.set(r.dispute_id, r);
  }
  for (const [caseId, r] of latestByCase) {
    const status = typeof r.status === "string" ? r.status.trim().toLowerCase() : null;
    const validation =
      typeof r.validation_status === "string" ? r.validation_status.trim().toLowerCase() : null;
    if (status === "failed" || validation === "failed") blockedCaseIds.add(caseId);
  }
}
const beforeGuard = population.length;
population = population.filter((p) => !blockedCaseIds.has(p.dispute_id));
const skippedByGuard = beforeGuard - population.length;
...
if (skippedByGuard > 0) {
  console.log(`  SKIPPED (latest package failed / validation_failed): ${skippedByGuard}`);
}
