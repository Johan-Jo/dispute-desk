      ...
      <div className="max-w-[880px] mx-auto px-4 py-6 sm:px-6 lg:px-8 pb-16">
        <div className="rounded-2xl border border-[#E2E8F0] bg-white p-6 sm:p-8 shadow-sm">
          ...
          <Section id="editor" title="Content Editor">
            ...
            <P>
              <Strong>Bulk backfill (operators):</Strong> To assign default pillar images for published articles that still have no featured URL, clone the repo, set <Code>SUPABASE_SERVICE_ROLE_KEY</Code> and <Code>PEXELS_API_KEY</Code>, then run <Code>npm run backfill:resources-images:dry</Code> (preview) and <Code>npm run backfill:resources-images</Code>. The script pulls images from the Pexels API into per-pillar pools and rotates through them in order for published articles in that pillar so cards do not all look identical. Pass <Code>--force</Code> to the script to replace URLs that were already set. Details: <Code>README.md</Code> and <Code>docs/technical.md</Code>.
            </P>
            <H3>Mobile editor</H3>
