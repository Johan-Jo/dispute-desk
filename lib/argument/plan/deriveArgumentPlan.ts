...
export function deriveCaseArgumentPlan(
  ...
  for (const candidate of input.candidates) {
    ...
    excluded.push({
      recordId: candidate.recordId,
      fieldKey: candidate.fieldKey,
      // Contract revision 1. Symmetric with `IncludedFact`: without it a reader
      // of a persisted snapshot cannot tell whether the excluded record was in
      // the module's `criticalCategories`, which is exactly what separates
      // `no_primary_argument` from `all_support_excluded`. The derivation knows
      // it right here — carrying it costs nothing and re-deriving it later is
      // impossible.
      factCategory: candidate.factCategory,
      reason,
      ...
      //
      // `MerchantReviewItem.reasonToken` became an `I18nToken` in revision 1
      // while `ExcludedFact.merchantReasonToken` stayed a KEY, deliberately:
      // the plan is persisted and a key round-trips, where a params bag would
      // freeze a rendered count into the snapshot. The key is taken here; the
      // merchant projection re-wraps it.
      merchantReasonToken: reviewItem?.reasonToken.key ?? exclusionReasonToken(reason),
    });
  }
