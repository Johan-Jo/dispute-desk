...
describe("deadline submit — a deadline relaxes NOTHING (P-6)", () => {
  ...
  it.each(["weak", "insufficient"])(
    "STRENGTH — a %s case IS filed at the deadline (revision 2)",
    async (overall) => {
      /*
       * THIS ASSERTION IS REVERSED FROM WHAT IT SAID, DELIBERATELY. It read
       * "a weak case is not filed at the deadline" and expected
       * `blockedByDecision: 1`. Contract revision 2 replaced that behaviour,
       * and the reasoning is worth restating where the reversal happened:
       *
       * filing nothing does NOT produce silence. Shopify auto-compiles and
       * files its own scrape when we file nothing, there is no accept/concede
       * mutation, and VDMP/VAMP compute the dispute ratio from disputes
       * RECEIVED — so losing a representment costs nothing. Withholding a weak
       * case therefore does not protect the merchant from a loss; it swaps our
       * document for Shopify's scrape and calls that caution.
       *
       * A hard block is an HONESTY condition — coverage/concession, fatal-loss,
       * or an unsafe claim. Thin evidence, argued honestly, is none of those,
       * and the cases either side of this one (hard block, coverage, staleness,
       * unsafe claim) still refuse exactly as they did.
       */
      const { rpc } = setup({
        packJson: { ...HEALTHY_PACK_JSON, case_strength: { overall } },
      });
      const body = await (await GET(req())).json();
      expect(rpc).toHaveBeenCalled();
      expect(body.enqueuedSubmit).toBe(1);
  ...
  it("names the failing P-6 conditions in the audit row", async () => {
    /*
     * Driven off a HARD BLOCK now, not a weak case. Revision 2 means a weak
     * case files, so it no longer produces a refusal for this assertion to
     * inspect — the diagnosability property being pinned is unchanged, only
     * the case that exercises it. A blocked readiness is the same refusal the
     * `HARD BLOCK` case above asserts, read one level deeper.
     */
    setup({ readiness: "blocked" });
    await GET(req());
    ...
    expect(payload.decisionReasonCodes).toEqual(["hard_block"]);
