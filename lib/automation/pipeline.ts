...
export async function evaluateAndMaybeAutoSave(packId: string): Promise<{
  ...
  // `hold_for_deadline` is the decision's name for what this function has
  // always called "parked by the Moderate guard": nothing is wrong with the
  // case, it simply waits for the clock rather than for a merchant.
  const parksAsModerate = ruleMode === "auto" && decision.action === "hold_for_deadline";
  ...
  if (
    ruleMode === "auto" &&
    decision.action === "block" &&
    decisionHead === "strength_insufficient"
  ) {
    if (isRegen && pack.dispute_id) {
  ...
  const gateReasons: string[] = decision.action === "auto_file" ? [] : [gateBlockMessage(decision.reasonCodes[0], settings, pack.completeness_score ?? 0)];
