...
import type { PersistedPackCompletenessRow } from "../completenessSnapshot";
...
/**
 * The production call site, exactly: read the persisted row through the
 * canonical reader, then hand the gate what it accepts. Written once so no
 * test can accidentally assert against a gate input production never builds.
 */
function gateInputFor(row: PersistedPackCompletenessRow) {
  const persisted = readPersistedCompletenessForGate(row);
  return {
    autoSaveEnabled: true,
    autoSaveMinScore: 50,
    enforceNoBlockers: true,
    completenessScore: persisted.score,
    blockers: persisted.blockers,
    submissionReadiness: readinessForGate(persisted.readiness),
  };
}
...
describe("readPersistedCompletenessForGate — the three faithful coercions", () => {
  it("NULL score → 0, so an unscored pack parks instead of auto-filing", () => {
    expect(readPersistedCompletenessForGate({ completeness_score: null }).score).toBe(0);
    expect(readPersistedCompletenessForGate({}).score).toBe(0);
    expect(readPersistedCompletenessForGate(null).score).toBe(0);
    // 0 is a real score and must survive — `?? 0` and `|| 0` differ here only
    // for 0 itself, and only `??` is correct.
    expect(readPersistedCompletenessForGate({ completeness_score: 0 }).score).toBe(0);
    expect(readPersistedCompletenessForGate({ completeness_score: 42 }).score).toBe(42);
  });
  ...
  it("NULL readiness → null, which selects the LEGACY blocker-count arm", () => {
    // The load-bearing one. CONTRACT REVISION 1 made this state representable
    // in `CompletenessSnapshot` itself, so the reader now returns `null` where
    // it used to return `undefined` on a parallel type. It is not a missing
    // value; it is a routing decision, and the two obvious "fixes" both change
    // dispositions:
    //   → "ready"   auto-files every legacy pack that has blockers
    //   → "blocked" freezes every legacy pack forever
    expect(readPersistedCompletenessForGate({ submission_readiness: null }).readiness)
      .toBeNull();
    expect(readPersistedCompletenessForGate({}).readiness).toBeNull();
    expect(readPersistedCompletenessForGate({ submission_readiness: "ready" }).readiness)
      .toBe("ready");
    expect(readPersistedCompletenessForGate({ submission_readiness: "blocked" }).readiness)
      .toBe("blocked");
  });

  it("readinessForGate spells the legacy arm the way the gate reads it", () => {
    // The gate branches on `submissionReadiness !== undefined`. `null` is the
    // contract's spelling of the same arm; this is the ONE conversion point.
    expect(readinessForGate(null)).toBeUndefined();
    expect(readinessForGate("ready")).toBe("ready");
  });
  ...
  it("an unrecognised readiness falls back to the legacy arm rather than clearing the gate", () => {
    ...
    expect(
      readPersistedCompletenessForGate({ submission_readiness: "almost_ready" }).readiness,
    ).toBeNull();
  });

  it("END TO END: the legacy arm still blocks on blocker count when readiness is NULL", () => {
    expect(evaluateAutoSaveGate(gateInputFor({
