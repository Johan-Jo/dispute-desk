...
export const visa_10_4_fraud: ReasonCodeGuidance = {
  ...
  promptBody: [
    ...
    "When an ip_location fact is in approvedFacts, the ONLY text you may write about it is a VERBATIM copy of that fact's `bankLocationSummary`. Do not paraphrase it, do not extend it, and do not describe the IP signal in your own words. If `bankLocationSummary` is absent or empty, write NOTHING about IP, geolocation or where the order was placed from. THE COMPARISON IS AGAINST THE SHIPPING ADDRESS AND NOTHING ELSE. Never say the IP agrees with billing, and never state or imply that the two order addresses correspond to one another — that is the retired agreement claim and it fails validation. Do NOT quote the raw IP address, the city name, the ISP/ASN, coordinates, or the IPinfo provider name: the bank cannot audit a raw IP, and it can audit a country-match outcome. Treat this as supporting corroboration of the authentication argument, never as primary or decisive evidence — IP geolocation is descriptive, not contractual.",
  ...
  /* v10 (2026-08-11): the IP guidance told the model to CITE the signal and
   * gave it an example, leaving the phrasing to the model. It wrote an
   * assertion about two addresses from a fact that names neither — the
   * retired agreement claim — and the package failed validation. Correcting
   * the example in v8 did not stop it, because the model was not copying an
