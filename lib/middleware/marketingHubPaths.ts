...
export const hubPublicPathRegex =
  /^\/(resources|templates|case-studies|glossary|blog|compare)(\/.*)?$/;

const localePrefixedHubPathRegex = new RegExp(
  `^\\/(${PATH_LOCALE_PREFIX_PATTERN})\\/(resources|templates|case-studies|glossary|blog|compare)(\\/.*)?$`,
);
...
export const enPrefixedHubPathRegex =
  /^\/en\/(resources|templates|case-studies|glossary|blog|compare)(\/.*)?$/;
