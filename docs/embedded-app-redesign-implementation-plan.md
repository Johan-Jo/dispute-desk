...
- **Our repo:** We do not render an in-iframe **horizontal route nav** (tabs). `AppNavSidebar` renders `s-app-nav` with `display: none` so Shopify can read it. **`EmbeddedAppChrome`** (`components/embedded/EmbeddedAppChrome.tsx`, used in `app/(embedded)/app/layout.tsx`) adds the Figma-style **brand row + feedback card** above page content — not a duplicate of Shopify’s Admin top bar.
