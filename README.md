...
**Embedded UI vs full-page mocks:** Designs that show the entire Shopify Admin (top bar, merchant sidebar, Apps nav) include **chrome that Shopify renders**. Under `/app/*`, DisputeDesk controls the **iframe body**, including **`EmbeddedAppChrome`** (brand row + dismissible feedback) in `app/(embedded)/app/layout.tsx`, then each route’s content. See [`docs/technical.md`](docs/technical.md) (Embedded app troubleshooting) and [`docs/shopify-app-review-checklist.md`](docs/shopify-app-review-checklist.md) (Design vs production).
...
Published articles at `/resources` (and localized paths) use a **BlogView-style** hub UI: **Figma Make–aligned** hero (same `--dd-hero-*` gradient + blobs as the marketing home), shared **marketing footer**, featured slots, topic row with counts, card thumbnails, and a full-width **article hero** when a featured image is set. In the admin content editor (**Metadata**), set **Featured image URL** and **Featured image alt** (stored on `content_items`; migration `featured_image_alt`). URLs may be absolute (`https://…`, e.g. Supabase Storage) or site paths such as `/images/...` under `public/`. Details: [`docs/technical.md`](docs/technical.md) (Resources Hub, code-first hub articles). To assign default **Pexels** hero images to published articles that still lack a URL, set `PEXELS_API_KEY` and run `npm run backfill:resources-images:dry` then `npm run backfill:resources-images` (service role in `.env.local`). The script varies images per article (pools per pillar; assignment order follows sorted `id` within each pillar so the grid cycles through the pool). To overwrite existing URLs (e.g. after expanding the pool), run `node scripts/backfill-resources-featured-images.mjs --force`.
