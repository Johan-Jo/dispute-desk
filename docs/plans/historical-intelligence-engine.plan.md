...
Status: **Phase A built on branch `feat/intelligence-engine-phase-a`** (approved 2026-07-21). Migration applied to **dev** (`vrpkgu`); audit logic validated against **prod Blume Box read-only** (RPC not yet deployed to prod). **Prod migration + real in-prod run remain a separate per-change approval** (CLAUDE.md #9). Phases B–E unchanged and unstarted. Surface decision: **internal `/admin/intelligence` first** (Tailwind, passkey auth, no 6-locale i18n); promote to embedded merchant tab only after validation. First production/validation case: **Blume Box** (resolved via its normal `shops` row — zero hardcoded identifiers).
...
- **Phase B:** baseline descriptive report (the §7.1 metrics with correct interval methods); `intelligence_recommendations` schema + wiring; descriptive opportunity detection; fuller Intelligence UI.
- **Phase C:** Policy Simulator (templates, scenario assumptions, financial ranges).
...
- **Phase E:** LLM explanation layer (admin-only English); decision logging surfaced; exportable Blume Box report.
