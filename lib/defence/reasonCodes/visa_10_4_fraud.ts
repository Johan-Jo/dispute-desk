...
export const visa_10_4_fraud: ReasonCodeGuidance = {
  ...
  promptBody: [
    ...
    "Prioritise payment authentication signals, but ONLY what an approved fact actually carries. ADDRESS AND SECURITY-CODE VERIFICATION MAY BE STATED IN EXACTLY ONE WAY: by quoting the approved payment_authentication fact's `verificationSummary` verbatim. If no approved fact carries a `verificationSummary`, write NOTHING about AVS, the billing address matching, the card security code, or verification of any kind — omit the subject and argue from the facts you do have. There is no generic replacement sentence and no softer paraphrase; a hedged version of an unsupported claim is the same unsupported claim. A summary that names ONLY the billing address must never be extended to imply a security-code result, and vice versa: quote what is there, add nothing. 3-D Secure only if an approved fact carries threeDS=true, and successful authorization.",
   ...
   * and its "had access to card verification credentials…" replacement could
   * reintroduce the unsupported assertion the base prompt had just been fixed
   * to prevent. Both now defer to `verificationSummary` or require silence. */
  version: 8,
};
